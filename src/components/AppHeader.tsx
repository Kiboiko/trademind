...
export default function AppHeader({ active = 'Dashboard' }: { active?: string }) {
    ...
    <header className="sticky top-0 z-50 bg-[#070b09]/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1280px] items-center justify-between px-6 py-3.5">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {items.map((l) => {
            const isActive = active === l.label || pathname === l.to;
            return (
              <Link
                key={l.label}
                to={l.to}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] py-1 pl-1 pr-3">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-zinc-400 to-zinc-700 text-xs font-semibold">
              A
            </span>
